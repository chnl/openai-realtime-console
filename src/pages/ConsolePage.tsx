const LOCAL_RELAY_SERVER_URL: string =
  ...
  type Action =
    | { type: 'START_AWAITING_RESPONSE' }
    | { type: 'END_AWAITING_RESPONSE' }
    | { type: 'START_PROCESSING_RESPONSE' }
    | { type: 'END_PROCESSING_RESPONSE' }
    | { type: 'START_RECORDING' }
    | { type: 'STOP_RECORDING' }
    | { type: 'SET_CAN_PUSH_TO_TALK'; payload: boolean };
  ...
  function reducer(state: State, action: Action): State {
    switch (action.type) {
      case 'START_AWAITING_RESPONSE':
        return { ...state, isAwaitingResponse: true };
      case 'END_AWAITING_RESPONSE':
        return { ...state, isAwaitingResponse: false };
      case 'START_PROCESSING_RESPONSE':
        return { ...state, isProcessingResponse: true };
      case 'END_PROCESSING_RESPONSE':
        return { ...state, isProcessingResponse: false };
      case 'START_RECORDING':
        return { ...state, isRecording: true, isAwaitingResponse: true };
      case 'STOP_RECORDING':
        return { ...state, isRecording: false };
      case 'SET_CAN_PUSH_TO_TALK':
        return { ...state, canPushToTalk: action.payload };
      default:
        return state;
    }
  }
  ...
  export function ConsolePage() {
    ...
    const connectConversation = useCallback(async () => {
      if (state.isProcessingResponse) {
        console.warn('Currently processing a conversation response.');
        return;
      }
      dispatch({ type: 'START_PROCESSING_RESPONSE' });
    ...
    const disconnectConversation = useCallback(async () => {
      ...
      setItems([]);
      setMemoryKv({});
      dispatch({ type: 'END_PROCESSING_RESPONSE' });
    ...
    const handleAnswer = useCallback(
      async (quizId: string, userAnswer: string) => {
        if (!currentQuizQuestion || state.isAwaitingResponse) {
          console.warn('handleAnswer: Already awaiting response or no current question.');
          return;
        }
  
        console.log('handleAnswer: Starting to process answer.');
        dispatch({ type: 'START_AWAITING_RESPONSE' });
        dispatch({ type: 'SET_CAN_PUSH_TO_TALK', payload: false });
  
        try {
          const response = await fetchWithRetry('https://quizshow.ference.ai/api/checkAnswers', {
            method: 'POST',
          ...
          console.log('handleAnswer: Resetting response flags.');
          dispatch({ type: 'SET_CAN_PUSH_TO_TALK', payload: true });
          dispatch({ type: 'END_AWAITING_RESPONSE' });
        }
      },
      [currentQuizQuestion, state.isAwaitingResponse]
    );
    ...
    const startRecording = useCallback(async () => {
      if (state.isAwaitingResponse || state.isProcessingResponse) {
        console.warn('startRecording: Already recording or processing response.');
        return;
      }
      console.log('startRecording: Initiating recording.');
    ...
    }, [state.isAwaitingResponse, state.isProcessingResponse]);
    ...
    const stopRecording = useCallback(async () => {
      if (!state.isRecording) {
        console.warn('stopRecording: Recording is not active, cannot stop.');
        return;
      }
    ...
    const changeTurnEndType = useCallback(async (value: string) => {
      ...
      dispatch({ type: 'SET_CAN_PUSH_TO_TALK', payload: value === 'none' });
    }, []);
    ...
    useEffect(() => {
      ...
      client.on('conversation.interrupted', async () => {
        ...
        console.log('conversation.interrupted: Resetting isProcessingResponse.');
        dispatch({ type: 'END_PROCESSING_RESPONSE' });
      });
      client.on('conversation.updated', async ({ item, delta }: any) => {
        ...
        if (item.status === 'completed') {
          console.log('conversation.updated: Response completed. Resetting isProcessingResponse.');
          dispatch({ type: 'END_PROCESSING_RESPONSE' });
        }
      });
    ...
    }, [isAuthorized]); // Updated dependency array to include isAuthorized
    ...
    /**
     * Implement Timeout for Awaiting Response
     */
    useEffect(() => {
      if (state.isAwaitingResponse) {
        const timer = setTimeout(() => {
          console.warn('Timeout: Resetting isAwaitingResponse due to inactivity.');
          dispatch({ type: 'END_AWAITING_RESPONSE' });
          dispatch({ type: 'SET_CAN_PUSH_TO_TALK', payload: true });
          ...
          <div className="content-main">
            <div className="content-logs">
              ...
              <div className="content-actions">
                ...
                {isConnected && state.canPushToTalk && (
                  <Button
                    label={state.isRecording ? 'release to send' : 'push to talk'}
                    buttonStyle={state.isRecording ? 'alert' : 'regular'}
                    disabled={!isConnected || !state.canPushToTalk}
